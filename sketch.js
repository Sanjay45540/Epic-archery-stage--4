...
function handlePlayerArrowColloision() {
  for(var i = 0; i<playerArrows.length ; i++){
    var baseColloision = Matter.SAT.colloides(
      playerArrows[i].body,
      computerBase.body
    );
    var archerColloision = Matter.SAT.colloides(
      playerArrows[i].body,
      computerArcher.body
    );
    var computerColloision = Matter.SAT.colloides(
      playerArrows[i].body,
      computer.body
    );
    if (
      baseColloision.colloided||
      archerColloision.colloided||
      computerColloision.colloided
    ){
      console.lof("Player arrow colloided")
    }
